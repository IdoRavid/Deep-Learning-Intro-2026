...
if __name__ == "__main__":
    ...
    samples_ode = model.sample(bsz=32, noise=noise, x0=deg_x, device=args.device, stochastic=False).cpu().numpy()
    samples_sde = model.sample(bsz=32, noise=noise, x0=deg_x, device=args.device, stochastic=True).cpu().numpy()

    samples_ode = rearrange(samples_ode, "t b () h w -> t b (h w)") * input_sd + input_mean
    samples_sde = rearrange(samples_sde, "t b () h w -> t b (h w)") * input_sd + input_mean

    x_vis_denorm = x_true * input_sd + input_mean

    def make_raster(samples):
        nrows, ncols = 10, 3
        percents = min(len(samples), 4)
        raster = np.zeros((nrows * 32, ncols * 32 * (percents + 1)), dtype=np.float32)
        ...
        for i in range(nrows * ncols):
            ...
            raster[32 * row: 32 * (row + 1), offset + 32 * col: offset + 32 * (col + 1)] = x_vis_denorm[i].reshape(32, 32)
        raster[:, ::32 * ncols] = 64
        return raster

    plt.imsave("./examples/ex_mnist_ode.png", make_raster(samples_ode), vmin=0, vmax=255, cmap='gray')
    plt.imsave("./examples/ex_mnist_sde.png", make_raster(samples_sde), vmin=0, vmax=255, cmap='gray')
